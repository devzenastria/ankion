-- Phase 28A - owner-controlled creation boundary.
-- Scope: controlled provisioning for profiles_private and anonymous_identities only.
-- Do not apply in Phase 28A. Static review and separate explicit GO are required before local apply.
-- No direct broad INSERT/UPDATE/DELETE grants are introduced.
...
begin
  ...
  values (
    v_owner_user_id,
    'Anonymous voice',
    'voice-' || encode(gen_random_bytes(8), 'hex'),
    'Voice ready'
  )
...
comment on function public.create_owner_identity_foundation(text, text, text) is
  'Controlled owner-bound provisioning boundary for the first private profile and active anonymous identity. Ownership is derived from auth.uid(); direct broad INSERT remains blocked.';
